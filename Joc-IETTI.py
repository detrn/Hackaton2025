...
def update_and_draw_particles(surface):
    # Actualizeaza si deseneaza particule
    for p in particles[:]:
        p['x'] += p['vx']
        p['y'] += p['vy']
        p['life'] -= 10
...
def reset_game():
    global score, start_ticks, game_active, player_amp
    score = 0
    game_active = True
...
while running:
    ...
    if game_active:
        ...
        bar_width = 300
        fill_width = int((time_left / time_limit) * bar_width)
